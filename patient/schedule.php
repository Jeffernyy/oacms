...
<body>
  ...
  // here is the portion of the code that I have added
  // you can edit this base on what is needed
  // chat me if there's something that I can assist you with
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // perform database insertion here (use prepared statements for security)
    $stmt = $database->prepare("INSERT INTO schedule (title, scheduledate, scheduletime) VALUES (?, ?, ?)");
    $stmt->bind_param("sss", $title, $scheduledate, $scheduletime);
              ...
              <table width="100%" class="sub-table scrolldown" border="0" style="padding: 50px;border:none">
                <tbody>
                  ...
                  if ($result->num_rows == 0) {
                    ...
                    // this portion of code that I have added
                    // chat me clarish/mali=macla ayiee
                    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                      // handle form submission and insert data into the database
                      $title = $_POST['title'];
                      $scheduledate = $_POST['scheduledate'];
                      $scheduletime = $_POST['scheduletime'];
                      // perform database insertion here (use prepared statements for security)
                      // after successful insertion, you can provide a success message or redirect to another page.
                      echo "Data inserted successfully!";
                    } else {
                      ...
                      echo '
                            <form method="post">
                              ...
                              <input class="input-text" type="date" name="scheduledate">
                              <input class="input-text" type="time" name="scheduletime">
